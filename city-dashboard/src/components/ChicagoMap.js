...
const customIcon = new L.Icon({
  iconUrl: 'https://fonts.gstatic.com/s/i/materialicons/location_on/v10/24px.svg',
  iconSize: [24, 24],
  iconAnchor: [12, 24],
});
...
function ChicagoMap() {
  ...
  useEffect(() => {
    Papa.parse('/smaller_crime.csv', {
      download: true,
      header: true,
      complete: (results) => {
        setCrimeData(results.data);
      }
    });
  }, []);

  const positions = [{'lat': 41.85182, 'lng': -87.745336, 'name': 'Cicero (Pink Line)'} ,
  ...
  const filteredPositions = selectedColor === 'all'
    ...
    const handleMarkerClick = (position) => {
      const crimeInfo = crimeData.find(crime => crime.station_name === position.name);
      setSelectedPosition({ ...position, crimeInfo });
    };
      ...
      <div style={{ width: '100%', height: '100vh' }}>
        ...
        >
          ...
          {filteredPositions.map((pos, index) => (
            <Marker key={index} position={[pos.lat, pos.lng]} icon={customIcon} eventHandlers={{ click: () => handleMarkerClick(pos) }}>
              <Popup>
                {pos.name}
              </Popup>
            </Marker>
        ...
        {selectedPosition && (
          <div className="info-card">
            ...
            {selectedPosition.crimeInfo ? (
              <>
                <p>Violence Score: {isNaN(selectedPosition.crimeInfo.violence_score) ? 'No data exists' : selectedPosition.crimeInfo.violence_score}</p>
                <p>Top Crimes: {selectedPosition.crimeInfo.top_crimes || 'No data exists'}</p>
              </>
            ) : (
              <p>whatever</p>
            )}
